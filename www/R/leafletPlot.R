...
leafletPlot <- function(besteSolution,Polygon1,best=1,RadiWak=1,IconW=50,
                        opaP=0.4,pal1="blue",bord=0,...) {
  # besteSolution=ResG;Polygon1=Reck;
...
+towgs84=0,0,0"
  ...
  turbine_icon <- iconList(
    turbine_icon = makeIcon(
      iconUrl = "www/extdata/WindTurbine.png", 
  ...
  map <- leaflet() %>%
    ...
    addPopups(mean(c(extent(Polygon1)[2],extent(Polygon1)[1])), (extent(Polygon1)[4] +0.0005), group = "Title",popup = 
              ...
              options = popupOptions(closeButton = T,autoPan = T,zoomAnimation = F,
                                     closeOnClick = T)) %>% 
    ...
    ## Add the Polygon
    addPolygons(data = Polygon1, group = "Polygon", color=pal1,weight=bord,
