...
genAlgo           <- function(Polygon1, Rotor, n, fcrR, referenceHeight, 
                              ...
                              sourceCCL,sourceCCLRoughness,updateProgress1=NULL){
  ...
  cat("################### History of Genetic Algorithm ###################\n")
  ...
  Grid1 <- GridFilter(shape = Polygon1,resol = resol2,prop = Proportionality, plotGrid = F);
  Grid <- Grid1[[1]]
  ...
  if (topograp == FALSE){
    cat("\nTopography and orography are not taken into account.")
    ...
    srtm <- local(raster::getData(name = 'SRTM', download = T, lon=extpol[1], lat=extpol[2]));
